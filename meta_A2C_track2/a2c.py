...
class Agent(AgentWithConverter):
    ...
    def meta_lr(self,env_name,id,N_TASK=1):
        # print(envs)
        ...
        new_obs = self.env[env_name].reset()
        ...
        while (step_inner <= self._training_param.max_step):
            ...
            new_obs, reward, done, info = self.env[env_name].step(act)
            ...
            if done or alive_steps == self._training_param.max_step or step_inner == self._training_param.max_step:
                ...
                with tf.GradientTape() as tape:  #记录所有涉及到可训练变量的操作，以便稍后计算梯度
                    # actor loss
    ...
    def random_task(self, N_task):
        ...
        for _ in range(N_task):
            ...
            self.env[mix_names[_]].chronics_handler.tell_id(random.randint(0, 119))

            print("ID of chronic current folder:", self.env[mix_names[_]].chronics_handler.real_data.get_id())
            # Reset the environment
            # env.reset()
            self.env[mix_names[_]].set_id(self.env[mix_names[_]].chronics_handler.real_data.get_id())
    ...
    def train(self,
        ...
        with tqdm(total=train_step, disable=False, miniters=1, mininterval=3) as pbar:
            ...
            while(step < train_step): #外循环
                ...
                for env_name, _ in env_list: #内循环
                    step_inner = 0  # 内循环的步数
                    done = False
                    # env_id = env[env_name]
                    ...
                    while(step_inner <= self._training_param.max_step):  # 设置每个环境跑max_step步数
                        ...
                        if done or alive_steps == self._training_param.max_step:
                            ...
                            if step_inner == self._training_param.max_step:  # 内层跑满max_step步进行元更新
                                # 进行元学习
                                self.meta_lr(env_name, _, len(env_list))
                        ...
                        if step % 100 == 0 and len(self.epoch_rewards) >= 1:
                            with train_summary_writer.as_default():
                                ...
                                if len(self.epoch_rewards) >= 50:
                                    ...
                                    mean_loss_actor_50 = np.mean(self.loss_actor_list[-50:])
                                    mean_loss_critic_50 = np.mean(self.loss_critic_list[-50:])

                                else:
                                    mean_reward_50 = mean_reward
                                    mean_alive_50 = mean_alive
                                    mean_loss_actor_50 = mean_loss_actor
                                    mean_loss_critic_50 = mean_loss_critic

                                if len(self.epoch_rewards) >= 1:
                                    tf.summary.scalar("mean_reward_50", mean_reward_50, step)
                                    tf.summary.scalar("mean_alive_50", mean_alive_50, step)
                                    tf.summary.scalar("mean_loss_actor_50", mean_loss_actor_50, step)
                                    tf.summary.scalar("mean_loss_critic_50", mean_loss_critic_50, step)
